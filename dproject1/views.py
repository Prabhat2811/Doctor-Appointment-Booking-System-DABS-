...
def doctor_dash(request,name):
    ...
    if q<1:
        return render(request,'doctor_dash1.html')
    else:
        u1=appointment.objects.filter(option_doctor=name)
        return render(request,'doctor_dash.html',{'n':u1})
...
def doctor_dash1(request):
    d=doc.objects.all()
    return render(request,'doctor_dash1.html',{'d':d})

def delete(request,id):
    d=doc.objects.get(id=id)
    d.delete()
    return redirect('../doctor_dash1')
